...
/** /api_call/Query an Account **/
/*need to make this to accept user input*/
router.get('/accounts', function (req, res) {
  ...
  // Set up API call (with OAuth2 accessToken)
  var url = config.api_uri + req.session.realmId + '/accounts/' . $_POST ['query'] /*set to dynamic*/
  console.log('Making API call to: ' + url)
  var requestObj = {
    url: url,
    headers: {
...
/** /api_call/selectAccountById **/
/*need to make this to accept user input*/
router.get('/selectAccountById', function (req, res) {
  var token = tools.getToken(req.session)
  if(!token) return res.json({error: 'Not authorized'})
  if(!req.session.realmId) return res.json({
    error: 'No realm ID.  QBO calls only work if the accounting scope was passed!'
  ...
  // Set up API call (with OAuth2 accessToken)
  var url = config.api_uri + req.session.realmId + '/account/' + '/1?/' /*set to dynamic*/
  console.log('Making API call to: ' + url)
  var requestObj = {
    url: url,
    headers: {
...
router.get('/accounts', function (req, res) {
  ...
  // Set up API call (with OAuth2 accessToken)
  var url = config.api_uri + req.session.realmId + '/account/' + '/?/' /*set to dynamic*/
  console.log('Making API call to: ' + url)
  var requestObj = {
    url: url,
    headers: {
